...
func generateTheme(schema *Schema, outPath string) error {
	...
	buf.WriteString("func NewTheme(tokens map[string]string) (*Theme, error) {\n")
	buf.WriteString("\ttheme := &Theme{}\n")
		...
		for _, tok := range comp.SupportedTokens {
			...
			if hasStatePrefix(tok) {
				for _, p := range prefixes {
					if strings.HasPrefix(tok, p) {
						matchedPrefix = p
						baseTok = strings.TrimPrefix(tok, p)
						break
					}
				}
			}
			...
			cat, prop := parseTokenCatProp(baseTok)
			mapKey := fmt.Sprintf("md.comp.%s.%s", strings.ReplaceAll(comp.Name, "-", "."), strings.ReplaceAll(tok, "-", "."))

			if matchedPrefix == "" {
				structPath := fmt.Sprintf("theme.%sTokens.%s.%s", compName, toCamelCase(cat), toCamelCase(prop))
				buf.WriteString(fmt.Sprintf("\tif val, ok := tokens[\"%s\"]; ok {\n", mapKey))
				buf.WriteString(fmt.Sprintf("\t\t%s = val\n", structPath))
				buf.WriteString("\t} else {\n")
				buf.WriteString(fmt.Sprintf("\t\tmissing = append(missing, \"%s\")\n", mapKey))
				buf.WriteString("\t}\n")
			} else {
				stateName := toCamelCase(strings.TrimSuffix(matchedPrefix, "-"))
				overlayStruct := fmt.Sprintf("%s%sOverlay", compName, stateName)
				fieldName := toCamelCase(cat) + toCamelCase(prop)

				buf.WriteString(fmt.Sprintf("\tif val, ok := tokens[\"%s\"]; ok {\n", mapKey))
				buf.WriteString(fmt.Sprintf("\t\tif theme.%sTokens.%s == nil {\n", compName, stateName))
				buf.WriteString(fmt.Sprintf("\t\t\ttheme.%sTokens.%s = &%s{}\n", compName, stateName, overlayStruct))
				buf.WriteString("\t\t}\n")
				buf.WriteString("\t\tv := val\n")
				buf.WriteString(fmt.Sprintf("\t\ttheme.%sTokens.%s.%s = &v\n", compName, stateName, fieldName))
				buf.WriteString("\t} else {\n")
				buf.WriteString(fmt.Sprintf("\t\tmissing = append(missing, \"%s\")\n", mapKey))
				buf.WriteString("\t}\n")
			}
	...
	buf.WriteString("\t\treturn nil, fmt.Errorf(\"missing required tokens: %s\", strings.Join(missing, \", \"))\n")
	buf.WriteString("\t}\n")
	buf.WriteString("\treturn theme, nil\n")
	buf.WriteString("}\n\n")
